...
- Continuity check: for subsequent queries in the same chat, call `siradig_session_status` first; only recover with one `siradig_login` retry chain if needed.
- Retry policy (skill): at most 1 automatic recovery retry on session instability; if it fails again, ask the user.
- For ARCA tasks, never switch to generic browser click/type automation as fallback after MCP failure; keep MCP flow.
